...
function maps(){
   ...
   {
    zoom:7,
    center:new google.maps.LatLng(-0.41692390440727056, 121.38311586894454),
        ...
        success: function(result)
        {
         ...
         // alert(data);
            $.each(data,function(key1) 
            {
                ...
                var tersalurkan = "Tersalurkan";
                var sekolah = "{{$sekolah->nama_sekolah}}";
               
               if (data[key1]['distribusi'] == tersalurkan){
                    var icon = "{{asset('assets/marker/hijau.png')}}";
                }

                else if (data[key1]['nama_sekolah'] == sekolah){
                    var icon = "{{asset('assets/marker/blue2.png')}}";
                }
