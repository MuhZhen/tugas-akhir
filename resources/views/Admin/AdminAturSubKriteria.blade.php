                                ...
                                <div class=""></div>
                                              ...
                                              <div class="form-group">
                                                  <select name="k" onchange="this.form.submit()" class="form-control">
                                                      @foreach($kriteria as $k)
                                                          <option value="{{$k->id}}" {{(request('k') == $k->id)?'selected':''}}> {{$k->nama}} </option>
                                                      @endforeach
                                                  </select>
                                              </div>
                                          </form>
                                        ...
                                        <div class="col d-sm-flex-inverse mb-4" >
                                          ...
                                          <h1 class="h3 mb-4 text-gray-800"></h1>
                                              <a href="{{url('/admin/atur/sub-kriteria/tambah')}}" class="d-sm-inline-block btn btn-sm btn-primary shadow-sm">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered" id="datatable" widht="100" cellspacing="0">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($TampilSubKriteria as $no => $item)
                                        <?php $no++ ;?>
                                        <tr>
                                            <td style="text-align:center">{{ $no }}</td>
                                            <td style="text-align:center">{{ $item->kriteria->nama}}</td>
                                            <td style="text-align:center">{{ $item->nama}}</td>
                                            <td style="text-align:center">{{ $item->bobot}}</td>
                                            
                                            <td style="text-align:center">

                                                <form action="{{ url('/admin/atur/sub-kriteria/'. $item->id).'/edit'}}" method="get" class="d-inline" >
                                                    <button class="btn btn-info btn-circle btn-sm">
                                                            <i class="fas fa-fw fa-wrench"></i>
                                                    </button>
                                                </form>

                                                <form action="{{url('/admin/atur/sub-kriteria/'.$item->id.'/hapus')}}" method="post" onsubmit="return confirm('Yakin Hapus Data?')" class="d-inline" >
                                                    @method('delete')
                                                    @csrf
