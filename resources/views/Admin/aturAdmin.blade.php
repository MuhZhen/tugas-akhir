                                ...
                                <div class="col"></div>
                                        <div class="col d-sm-flex-inverse mb-4" >
                                          ...
                                          <h1 class="h3 mb-4 text-gray-800"></h1>
                                              <a href="{{url('/admin/atur/tambah')}}" class="d-sm-inline-block btn btn-sm btn-primary shadow-sm">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered" id="datatable" widht="100" cellspacing="0" >
                                    ...
                                    <tbody>
                                       ...
                                       <tr>
                                            <td style="text-align:center">{{ $pengguna->firstitem()+$no }}</td>
                                            <td style="text-align:center">{{ $item->username}}</td>
                                            <td style="text-align:center">{{ $item->nama_asli}}</td>
                                            <td style="text-align:center">{{ $item->alamat}}</td>
                                            <td style="text-align:center">{{ $item->no_telpon}}</td>
                                            <td style="text-align:center">{{ $item->email}}</td>
                                            <td style="text-align:center">{{ $item->level_akses}}</td>

                                            <td style="text-align:center">
                                                <form action="{{ url('admin/atur/edit/'. $item->id)}}" method="get" class="d-inline" >
                                                    <button class="btn btn-info btn-circle btn-sm">
                                                            <i class="fas fa-fw fa-wrench"></i>
                                                    </button>
                                                </form>

                                                <form action="{{url('admin/atur/delete/'.$item->id)}}" method="post" onsubmit="return confirm('Yakin Hapus Data?')" class="d-inline" >
                                                    @method('delete')
                                                    @csrf
