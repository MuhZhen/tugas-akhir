            ...
            <div class="card shadow mb-4">
                  ...
                  <div class="card-body">
                        ...
                        <div class="text-center">
                            <h1 class="h4 text-gray-900 mb-4">Tambah Sub Kriteria</h1>
                        </div>

                        <div class="col-lg-8 container">
                            <form action="{{url('/admin/atur/sub-kriteria/tambah')}}" method="post">
                                @csrf
                                <div class="form-group">
                                    <label>Nama Kriteria</label>
                                    <select name="kriteria_id" class="form-control" >
                                        <option value="">--Pilih Kriteria--</option>
                                        @foreach ($TampilDataKriteria as $item)
                                        <option value="{{ $item->id }}">{{ $item->nama }}</option>
                                        @endforeach
                                    </select>
                                    <label @error('kriteria_id') class="text-danger" @enderror> @error('kriteria_id')
                                  {{$message}}
                                @enderror</label>
                                </div>
