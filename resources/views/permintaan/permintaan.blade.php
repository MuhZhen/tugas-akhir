...
<body id="page-top">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Data Permintaan Sekolah</h1>
          </div>
            ...
            <div class="card shadow mb-4">
                ...
                @endif
                  ...
                  <div class="card-body">
                        ...
                        <form action="{{url('/admin/permintaan/cari')}}" method="GET" class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
                              <div class="input-group">
                                <input type="text" name="cari" value="{{ old('cari') }}" class="form-control bg-light border-0 small" placeholder="Nomor Induk Sekolah" aria-label="Search" aria-describedby="basic-addon2">
                            ...
                            <div class="col d-sm-flex-inverse mb-4" >
                                          ...
                                          <h1 class="h3 mb-4 text-gray-800"></h1>
                                              <a href="" data-target="#tambahModal" data-toggle="modal" class="d-sm-inline-block btn btn-sm btn-primary shadow-sm">
                                                  <i class="fas fa-plus fa-sm text-white-50"></i> Tambah Data
                                              </a>  

                                              &nbsp;

                                              <a href="" data-target="#hapusGelombang" data-toggle="modal" class="d-sm-inline-block btn btn-sm btn-danger shadow-sm">
                                                  <i class="fas fa-trash fa-sm text-white-50"></i> Hapus Data Golombang
                                              </a> 
                                ...
                                <table class="table table-bordered" id="datatable" widht="100" cellspacing="0" >
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="text-align:center">Nomor Induk Sekolah</th>
                                            <th style="text-align:center">Bantuan</th>
                                            <th style="text-align:center">Periode</th>
                                            <th style="text-align:center">Action</th>
                                        </tr>
                                    </thead>
                                            @foreach ($tampilBantuan as $no => $item)
                                            
                                        <tr>
                                            
                                            <td style="text-align:center">{{ $tampilBantuan->firstitem()+$no }}</td>
                                            <td style="text-align:center">{{ $item->nama_sekolah}}</td>
                                            <td style="text-align:center">{{ $item->NPSN}}</td>
                                            <td style="text-align:center">{{ $item->bantuan}}</td>
                                            <td style="text-align:center">{{ $item->nama_periode}}</td>
                                            <td style="text-align:center">
                                                
                                                <form action="{{ url('/admin/permintaan/tampilEdit/'. $item->id)}}" method="get" class="d-inline" >
                                                    <button class="btn btn-info btn-circle btn-sm">
                                                            <i class="fas fa-fw fa-wrench"></i>
                                                    </button>
                                                </form>

                                                <form action="{{url('/admin/permintaan/delete/'.$item->id)}}" method="post" onsubmit="return confirm('Yakin Hapus Data?')" class="d-inline" >
                                                    @method('delete')
                                                    @csrf
 ...
 <div class="modal fade" id="hapusGelombang" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalCenterTitle">Pilih Gelombang</h5>
      ...
      <form action="{{url('admin/permintaan/semua/hapus')}}" method="POST">
      @csrf
                              <div class="form-group">
                                    <label>Periode</label>
                                    <select name="periode" class="form-control" >
                                        @foreach ($tampilPeriode as $item)
                                ...
                                <button class="btn btn-danger" type="submit" onclick="return confirm('Apakah Kamu Yakin Menghapus Ini?')">Hapus</button>
 ...
 <div class="modal fade" id="tambahModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <a href="{{url('/admin/permintaan/tambah/tabel')}}" type="button" class="btn btn-primary btn-lg btn-block">Tambah Berdasarkan Sekolah</a>
      <a href="{{url('admin/permintaan/tampiltambahsemua')}}" type="button" class="btn btn-success btn-lg btn-block">Tambah Semua</a>
